...
const Courses: React.FC<{programID: string}> = ({programID}) => {
  ...
  useEffect(() => {
    ...
    api.get<CourseResponse[]>('/courses', {params: {programID}}).then(res => res.data).then(setCourses);
  }, []);
  return (
    <Layout>
      ...
      {courses.map((course) => (
        <div key={course.courseID} className="rounded shadow-lg p-3">
          <Link to={`./${course.courseID}`}>{course.courseName} - {course.semester == 3?'S':course.semester}/{course.year}</Link>
        </div>
      ))}
